...
class UNET_ResidualBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, n_time=1280):
        ...
        # 为了residue和merged可以相加，他们的维度必须一样
...
class UNET_AttentionBlock(nn.Module):
    ...
    def forward(self, x, context):
        ...
        # Normalization + FF + GEGLU with skip connection
...
class Diffusion(nn.Module):
    ...
    def forward(self, latent: torch.Tensor, context: torch.Tensor, time: torch.Tensor):
        ...
        # unet是一个循环的过程，它不断的接收一个有噪声的图片，判断有多少噪声，去噪声，
        # 再重复这个过程
        # (Batch_size, 4, height/8, weight/8) --> (Batch_size, 320, height/8, weight/8)
        output = self.unet(latent, context, time)

        # (Batch_size, 320, height/8, weight/8) --> (Batch_size, 4, height/8, weight/8)
